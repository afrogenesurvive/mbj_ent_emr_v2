...
import VisitItem from '../../items/user/VisitItem';
import './UserList.css';

const UserVisitList = props => {
  ...
  if (filter.field === 'visit' && filter.key === 'followUp') {
    propsVisits = visits2.filter(x => x.followUp === filter.value);
  }
  ...
  return <ul className="appointmentList">{visits}</ul>;
};
